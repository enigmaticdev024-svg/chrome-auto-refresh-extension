document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load saved settings
  chrome.storage.sync.get(['theme', 'interval', 'randomize', 'alertEnabled', 'stopPrompt', 'monitorMode'], (data) => {
    // Set theme
    if (data.theme === 'light') {
    ...
    if (data.stopPrompt) {
      stopPromptInput.value = data.stopPrompt;
    }

    if (data.monitorMode) {
      monitorModeSelect.value = data.monitorMode;
    }
  });
  ...
  toggleButton.addEventListener('click', () => {
    if (toggleButton.classList.contains('start')) {
      ...
      chrome.runtime.sendMessage({ action: 'stop', autoStopped: false }, (response) => {
